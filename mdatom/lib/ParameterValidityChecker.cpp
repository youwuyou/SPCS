...
bool ParameterValidityChecker::checkValidity() {
    ...
    if (par.xvInitialization == InitialXVGenerator::generateInitialX &&
        (par.numberAtomsOnBoxEdge[0] == 0 || par.numberAtomsOnBoxEdge[1] == 0 || par.numberAtomsOnBoxEdge[2] == 0)) {
    ...
    if (2 * par.interactionCutoffRadius > par.boxSize[0] || 2 * par.interactionCutoffRadius > par.boxSize[1] ||
        2 * par.interactionCutoffRadius > par.boxSize[2]) {
    ...
    if (par.initialTemperature >= 1e-6
        && (par.xvInitialization == InitialXVGenerator::xvFromAscii ||
            par.xvInitialization == InitialXVGenerator::xvFromBinary)) {
